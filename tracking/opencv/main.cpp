    ...
    while (cap.read(image))
    {
        ...
        if (frame_count % infer_rate == 0)
        {
            ...
            for (size_t i = 0; i < objs.size(); i++)
            {
                ...
                trackerInfo.id = i + 1; // Assigner un ID de suivi unique à chaque tracker

                trackers.push_back(trackerInfo);

                // Dessiner l'ID de suivi sur l'image
                cv::putText(image, "ID: " + std::to_string(trackerInfo.id), cv::Point(bbox.x, bbox.y - 5), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 2);
            }
        }
        else
        {
            // Mettre à jour les trackers avec le nouveau frame
            for (auto& trackerInfo : trackers)
            {
                cv::Rect bbox; // Créer un objet nommé au lieu de passer un objet temporaire et changer le type en cv::Rect
                bool ok = trackerInfo.tracker->update(image, bbox);
                if (ok)
                {
                    // L'objet est toujours suivi. Dessiner la nouvelle boîte englobante.
                ...
                {
                    // Le suivi a échoué. Exécuter à nouveau l'inférence YOLOv8 pour détecter à nouveau l'objet.
                    yolov8->infer();
                    yolov8->postprocess(objs);

                    // Initialiser le tracker avec le premier objet détecté
                    if (!objs.empty())
                    {
                        Object obj = objs[0];
                        cv::Rect bbox(obj.rect.x, obj.rect.y, obj.rect.width, obj.rect.height);
