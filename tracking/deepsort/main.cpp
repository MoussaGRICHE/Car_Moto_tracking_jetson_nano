...
public:
    void run(cv::Mat& img_rgb, std::vector<Object>& objs) {
        // Convert YOLOv8 detections to DeepSort detections
        std::vector<DetectBox> boxes;
        for (const auto& obj : objs) {
            DetectBox box(obj.rect.x, obj.rect.y, obj.rect.x + obj.rect.width, obj.rect.y + obj.rect.height, obj.prob, obj.label);
            boxes.push_back(box);
        }
        // Track objects using DeepSort
        DS->sort(img_rgb, boxes);
        // Update YOLOv8 detections with tracking information
        for (size_t i = 0; i < boxes.size(); i++) {
            objs[i].track_id = boxes[i].trackID;
        }
    }
...
int main(int argc, char** argv)
{
    ...
    Tester* test = new Tester(engine_file_path);
	...
	while (cap.read(image))
	{
        ...
        // Convert BGR image to RGB for DeepSort
        cv::Mat img_rgb;
        cv::cvtColor(image, img_rgb, cv::COLOR_BGR2RGB);
        // Track objects using DeepSort
        test->run(img_rgb, objs);
