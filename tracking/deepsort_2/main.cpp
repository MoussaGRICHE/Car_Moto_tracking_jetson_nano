	...
	while (cap.read(image))
	{
       ...
       // Convert objects to detections
        ...
        for (const auto& obj : objs)
        {
            ...
            detection.tlwh = obj.to_tlwh(); // Assuming to_tlwh() returns DETECTBOX
            detection.confidence = obj.prob;
            // Set feature if available: detection.feature = obj.feature;
        ...
        for (const auto& track : mytracker.tracks) {
            if (!track.is_confirmed() || track.time_since_update > 1)
                continue;

            DETECTBOX tlwh = track.to_tlwh();
            cv::Rect box(tlwh(0), tlwh(1), tlwh(2), tlwh(3));
            cv::rectangle(res, box, cv::Scalar(0, 0, 255), 2);
            std::string label = cv::format("%d", track.track_id);
            cv::putText(res, label, cv::Point(box.x, box.y), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255), 2);
        }
