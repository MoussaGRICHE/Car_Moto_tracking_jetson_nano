...
int main(int argc, char** argv) {
    ...
    while (cap.read(image))
    {
        ...
        // Track objects
        dets.clear();
        float bbox[4] = {0.0f}; // Bounding box coordinates
        // Iterate over the objs vector and extract the necessary data
        for (const auto& obj : objs) {

            bbox[0] = obj.rect.x + obj.rect.width / 2.0f; // x-coordinate of the center
            bbox[1] = obj.rect.y + obj.rect.height / 2.0f; // y-coordinate of the center
            bbox[2] = obj.rect.width; // Width
            bbox[3] = obj.rect.height; // Height
        ...
        for (auto& trk : tracks) {
            ...
            cv::rectangle(image, bbox, cv::Scalar(0x27, 0xC1, 0x36), 2);
            
            // Display tracker ID and object label
            std::string label = std::to_string(trk.first);
            cv::putText(image, label, cv::Point(bbox.tl().x, bbox.tl().y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
        }
